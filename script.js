...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `PUNTEGGIO: ${state.game.score}`;
		scoreNode.style.display = 'block';
		cubeCountNode.style.opacity = 0.65 ;
	}
	cubeCountNode.innerText = `CUBI ROTTI: ${state.game.cubeCount}`;
}
...
function renderMenus() {
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			if (isNewHighScore()) {
				highScoreLblNode.textContent = 'Nuovo record!';
			} else {
				highScoreLblNode.textContent = `Punteggio alto: ${formatNumber(getHighScore())}`;
			}
